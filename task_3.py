...
def load_logs(log_path: Path):
	...
	Args:
		log_path (Path): шлях до файлу з логами
	Returns:
...
def parse_log_line(line: str) -> dict:
	...
	return {key: value for key, value in zip(['date', 'time', 'level', 'message'], splitted)}
...
def filter_logs_by_level(logs: list, level: str):
	...
	if not level:
		return []
	logs_level = [log for log in logs if log['level'].lower() == level.lower()]
	return logs_level
...
def main(argv):
	...
	details_level = filter_logs_by_level(logs, level) # фільтрація логів за рівнем

	# відображення результатів
	print(display_log_counts(counts))

	# деталі логів для вказаного рівня
	if level and details_level: # якщо рівень вказано та є логи для цього рівня
